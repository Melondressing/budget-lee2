...
const state = {
  ...
  currentUser: null,
  authToken: localStorage.getItem('auth_token') || localStorage.getItem('authToken') || null,
  dynamicCategories: {
    income: [],
    expense: [],
    savings: []
...
function syncStoredAuthToken(token) {
  if (!token) return;
...
function formatCurrencyByCode(amount, currencyCode = state.settings.currency || 'KRW') {
  const code = String(currencyCode || state.settings.currency || 'KRW').toUpperCase();
  const symbol = CURRENCIES[code]?.symbol || `${code} `;
  return `${symbol}${Number(amount || 0).toLocaleString()}`;
}

function formatInvestmentMoney(amount, currencyCode = state.settings.currency || 'KRW') {
  const value = Number(amount || 0);
  const code = String(currencyCode || state.settings.currency || 'KRW').toUpperCase();
  const symbol = CURRENCIES[code]?.symbol || `${code} `;
  const maximumFractionDigits = ['KRW', 'JPY'].includes(code) ? 0 : (Math.abs(value) < 1 ? 6 : 2);
...
function formatInvestmentQuantity(quantity) {
  return Number(quantity || 0).toLocaleString(undefined, {
    minimumFractionDigits: 0,
...
async function checkAuth() {
  ...
  try {
    ...
    if (response.data.success && response.data.user && !response.data.user.isGuest) {
      state.isAuthenticated = true;
      state.currentUser = response.data.user;
      syncStoredAuthToken(token);
      return true;
    }
  } catch (error) {
  ...
  contentArea.innerHTML = `
    <div class="space-y-6">
      ...
      <div class="bg-sky-50 border border-sky-200 rounded-2xl p-4">
        <div class="flex items-start gap-3">
          <i class="fas fa-satellite-dish text-sky-600 mt-1"></i>
          <div class="text-sm text-sky-900">
            ...
            <p id="investment-price-status" class="mt-2 text-xs text-sky-700"></p>
          </div>
        </div>
      </div>
...
async function updateInvestmentPrices() {
  ...
  try {
    ...
    if (priceResponse.data.success) {
      quotes = priceResponse.data.data || {};
      quoteErrors = priceResponse.data.errors || {};
      quoteCacheMap = priceResponse.data.cached || {};
    }
  } catch (error) {
    console.error('Failed to fetch investment prices:', error);
    quoteErrors = symbols.reduce((acc, symbol) => {
      acc[symbol] = '주가 정보를 불러올 수 없습니다.';
      return acc;
    }, {});
  }
  ...
  for (const inv of state.investments) {
    ...
    const cachedBadge = quoteCacheMap[symbol]
      ? `<span class="ml-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700" title="${getLanguage() === 'ko' ? '60초 캐시된 실제 시세' : 'Cached live quote for 60 seconds'}">cached</span>`
      : '';
  ...
  if (totalProfitLossEl) {
    totalProfitLossEl.innerHTML = `
      <span class="${profitClass}">${profitSign}${formatInvestmentMoney(Math.abs(totalProfitLoss), summaryCurrency)}</span>
      <span class="text-sm ${profitClass}"> (${profitSign}${totalProfitLossPercent}%)</span>
    `;
  }
  if (priceStatusEl) {
    const nowText = new Date().toLocaleTimeString(getLanguage() === 'ko' ? 'ko-KR' : 'en-US', {
      hour: '2-digit',
      minute: '2-digit'
    });
    priceStatusEl.textContent = getLanguage() === 'ko'
...
function startInvestmentPriceRefresh() {
  ...
  // 60초마다 주가 업데이트. 서버 캐시 TTL과 맞춰 외부 호출을 줄인다.
        ...
        <form onsubmit="handleInvestmentSubmit(event, ${investmentId})" class="space-y-4">
          ...
          <div>
            ...
            <input type="number" name="quantity" value="${investment?.quantity || ''}"
                   placeholder="Number of shares" required min="0.00000001" step="any"
                   class="w-full px-4 py-2 border rounded">
            <p class="text-xs text-gray-500 mt-1">${getLanguage() === 'ko' ? '소수점 수량도 입력할 수 있어요. 예: 0.025 BTC, 1.5 shares' : 'Fractional quantities are supported, e.g. 0.025 BTC or 1.5 shares.'}</p>
          </div>
...
async function checkLoginStatus() {
  ...
  if (authToken) {
    ...
    if (userNameEl) userNameEl.textContent = userName || (userEmail ? userEmail.split('@')[0] : 'User');
    ...
    try {
      ...
      if (response.data.success && response.data.user) {
        ...
        if (response.data.user.email) {
          ...
          if (userEmailEl) userEmailEl.textContent = response.data.user.email;
        }
        if (response.data.user.name) {
          localStorage.setItem('user_name', response.data.user.name);
          if (userNameEl) userNameEl.textContent = response.data.user.name;
        }
      }
    } catch (error) {
